...
# Dependency to get PayPal service
async def get_paypal_service(
    request: Request
) -> PayPalService:
    """Get PayPal service instance with credentials from global config."""
    ...
    if not client_id or not client_secret or not redirect_uri:
        raise HTTPException(
            ...
            detail="PayPal OAuth credentials not configured in global_config. "
                   "Please configure: PAYPAL_OAUTH_CLIENT_ID, PAYPAL_OAUTH_CLIENT_SECRET, "
                   "PAYPAL_OAUTH_REDIRECT_URI in the CRM global configuration."
        )
...
async def complete_authorization(
    ...
    This endpoint handles the OAuth callback from PayPal.
    """
    try:
...
async def get_configuration(
    ...
    try:
        ...
        if not config:
            ...
            logger.info(f"PayPal integration not found for agent {agent_id}, creating default config")
            default_config = {
                "provider": "paypal",
                "connected": False,
            }
        ...
        logger.debug(f"PayPal config BEFORE sanitize for agent {agent_id}: has_access_token={has_access_token}, connected={config.get('connected')}, access_token present={bool(config.get('access_token'))}")
        ...
        logger.info(f"PayPal config for agent {agent_id}: connected={sanitized_config.get('connected')}, has_access_token={has_access_token}, config keys={list(sanitized_config.keys())}")
        return success_response(
            data=sanitized_config,
            message="Configuration retrieved successfully"
        )
    except Exception as e:
        logger.error(f"Error getting PayPal configuration: {e}")
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
async def save_configuration(
    ...
    try:
        ...
        if not stored_config:
            ...
            message="PayPal integration not connected. Please connect first via OAuth.",
        ...
        logger.info(f"Saved PayPal configuration for agent {agent_id}: connected={updated_config.get('connected')}")
        ...
        logger.error(f"Error saving PayPal configuration: {e}", exc_info=True)
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
async def disconnect(
    ...
    try:
        ...
        logger.error(f"Error disconnecting PayPal: {e}")
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
# Fixed OAuth Callback Endpoint (for PayPal OAuth App redirect)
@callback_router.get(
    "/callback",
    response_model=SuccessResponse[OAuthCallbackResponse],
    responses={
...
async def oauth_callback(
    ...
    try:
        ...
        # Get PayPal service instance (create without user token for callback)
        from src.services.global_config_service import get_global_config_service
        ...
        if not client_id or not client_secret or not redirect_uri:
            ...
            request=request,
            code=map_status_to_error_code(status.HTTP_400_BAD_REQUEST),
            message="PayPal OAuth credentials not configured. ",
            status_code=status.HTTP_400_BAD_REQUEST
        )
