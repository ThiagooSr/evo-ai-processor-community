...
router = APIRouter(
    prefix="/agents/{agent_id}/integrations/notion",
    tags=["notion"],
)
...
async def get_notion_service(
    ...
    """Get Notion service instance with credentials from global config."""
    ...
    credentials = await config_service.get_notion_credentials()
    ...
    if not redirect_uri:
        raise HTTPException(
            ...
            detail="Notion OAuth redirect_uri not configured in global_config. "
                   "Please configure: NOTION_OAUTH_REDIRECT_URI in the CRM global configuration. "
...
async def discover_oauth(
    ...
    """
    Discover OAuth requirements from Notion MCP server.
...
async def complete_authorization(
    ...
    This endpoint handles the OAuth callback from Notion.
    """
    try:
...
async def get_configuration(
    ...
    """Get Notion integration configuration. Creates default config if not found."""
    try:
        ...
        config = await get_integration_config(db, agent_id, "notion", load_from_service=service._load_credentials)
        
        if not config:
            # Create default configuration if not found
            logger.info(f"Notion integration not found for agent {agent_id}, creating default config")
            default_config = {
                "provider": "notion",
                "connected": False,
            }
            ...
            try:
                ...
                success = await upsert_agent_integration(
                    db, agent_id, "notion", default_config
                )
                if success:
                    logger.info(f"Created default Notion integration config for agent {agent_id}")
            except Exception as save_error:
                logger.warning(f"Could not save default Notion config: {save_error}")
        ...
        logger.debug(f"Notion config BEFORE sanitize for agent {agent_id}: has_access_token={has_access_token}, connected={config.get('connected')}, access_token present={bool(config.get('access_token'))}")
        ...
        logger.info(f"Notion config for agent {agent_id}: connected={sanitized_config.get('connected')}, has_access_token={has_access_token}, config keys={list(sanitized_config.keys())}")
        return success_response(
            data=sanitized_config,
            message="Configuration retrieved successfully"
        )
    except Exception as e:
        logger.error(f"Error getting Notion configuration: {e}")
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
async def discover_tools(
    ...
    """Discover available MCP tools from Notion using stored access_token."""
    from src.api.mcp_integration_base import discover_tools_endpoint
    tools = await discover_tools_endpoint(
        agent_id, "notion", "https://mcp.notion.com/mcp", db
    )
    return success_response(
        data=tools,
...
async def save_configuration(
    ...
    try:
        ...
        stored_config = await get_integration_config(db, agent_id, "notion", load_from_service=service._load_credentials)
        
        if not stored_config:
            return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_404_NOT_FOUND),
            message="Notion integration not connected. Please connect first via OAuth.",
        ...
        success = await upsert_agent_integration(
            db, agent_id, "notion", updated_config
        )
        ...
        logger.info(f"Saved Notion configuration for agent {agent_id}: connected={updated_config.get('connected')}")
        ...
        logger.error(f"Error saving Notion configuration: {e}", exc_info=True)
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
# Fixed OAuth Callback Endpoint (for Notion OAuth App redirect)
@callback_router.get(
    "/callback",
    response_model=SuccessResponse[OAuthCallbackResponse],
    responses={
...
async def oauth_callback(
    ...
    """
    OAuth 2.0 callback endpoint for Notion MCP (fixed URL).

    This endpoint handles the OAuth redirect from Notion.
    The agent_id is extracted from the state parameter.

    Query Parameters:
        code: Authorization code from Notion OAuth
    ...
    try:
        ...
        # Get Notion service instance (create without user token for callback)
        from src.services.global_config_service import get_global_config_service
        ...
        if not redirect_uri:
            ...
            request=request,
            code=map_status_to_error_code(status.HTTP_400_BAD_REQUEST),
            message="Notion OAuth redirect_uri not configured",
            status_code=status.HTTP_400_BAD_REQUEST
        )
