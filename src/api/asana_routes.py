...
# Dependency to get Asana service
async def get_asana_service(
    request: Request
) -> AsanaService:
    """Get Asana service instance with credentials from global config."""
    ...
    credentials = await config_service.get_asana_credentials()
...
async def get_asana_service_optional(
    request: Request
) -> Optional[AsanaService]:
    """Get Asana service instance, returning None if credentials are not configured."""
    try:
        return await get_asana_service(request)
    except HTTPException as e:
        if e.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR:
            logger.warning(f"Asana service not available: {e.detail}")
            return None
        raise
...
async def discover_oauth_requirements(
    ...
    try:
        ...
        return success_response(data=metadata, message="OAuth requirements discovered successfully")
...
async def complete_authorization(
    ...
    This endpoint handles the OAuth callback from Asana.
    """
    try:
...
async def get_configuration(
    ...
    try:
        ...
        config = await get_integration_config(db, agent_id, "asana", load_from_service=load_from_service)
        ...
        logger.debug(f"Asana config BEFORE sanitize for agent {agent_id}: has_access_token={has_access_token}, connected={config.get('connected')}, access_token present={bool(config.get('access_token'))}")
        ...
        logger.info(f"Asana config for agent {agent_id}: connected={sanitized_config.get('connected')}, has_access_token={has_access_token}, config keys={list(sanitized_config.keys())}")
        return success_response(
...
async def discover_tools(
    ...
    discovery_tools = await discover_tools_endpoint(
        agent_id, "asana", "https://mcp.asana.com/mcp", db, load_from_service=load_from_service
    )
...
async def update_configuration(
    ...
    try:
        ...
        stored_config = await get_integration_config(db, agent_id, "asana", load_from_service=service._load_credentials)
        ...
        success = await upsert_agent_integration(
            db, agent_id, "asana", updated_config
        )
        ...
        logger.info(f"Saved Asana configuration for agent {agent_id}: connected={updated_config.get('connected')}")
...
async def disconnect(
    ...
    try:
        ...
        logger.error(f"Error disconnecting Asana: {e}")
        return error_response(
            request=Request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
# Fixed OAuth Callback Endpoint (for Asana OAuth App redirect)
@callback_router.get(
    "/callback",
    response_model=SuccessResponse[OAuthCallbackResponse],
    responses={
...
async def oauth_callback(
    ...
    """
    OAuth 2.0 callback endpoint for Asana MCP (fixed URL).

    This endpoint handles the OAuth redirect from Asana.
    The agent_id is extracted from the state parameter.

    Query Parameters:
        code: Authorization code from Asana OAuth
    ...
    try:
        ...
        # Get Asana service instance (create without user token for callback)
        from src.services.global_config_service import get_global_config_service
        ...
        logger.info(
            f"Asana callback: Starting authorization completion. "
            f"agent_id={agent_id}"
        )
        ...
        logger.info(
            f"Asana callback: Authorization completed. "
        ...
        if result.get("success"):
            try:
                ...
                if credentials:
                    ...
                    logger.info(
                        f"Asana callback: Credentials verification. "
                        f"has_access_token={has_token}, "
                        f"config_keys={list(credentials.keys())}"
                    )
                    if not has_token:
                        logger.error(
                            f"Asana callback: WARNING - Authorization succeeded but access_token was not saved! "
                            f"Config: {credentials}"
                        )
                else:
                    logger.error(
                        f"Asana callback: WARNING - Authorization succeeded but no credentials found!"
                    )
            except Exception as verify_error:
                logger.error(f"Asana callback: Error verifying credentials: {verify_error}")
