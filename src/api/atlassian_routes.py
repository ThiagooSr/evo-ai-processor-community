"""
Atlassian integration API routes.
...
# Dependency to get Atlassian service
async def get_atlassian_service(
    request: Request
) -> AtlassianService:
    """Get Atlassian service instance with credentials from global config."""

    # Fetch credentials from global config
    config_service = get_global_config_service()
    credentials = await config_service.get_atlassian_credentials()
    ...
    if not redirect_uri:
        raise HTTPException(
            ...
            detail="Atlassian OAuth redirect_uri not configured in global_config. "
                   "Please configure: ATLASSIAN_OAUTH_REDIRECT_URI in the CRM global configuration. "
...
async def get_atlassian_service_optional(
    request: Request
) -> Optional[AtlassianService]:
    """Get Atlassian service instance, returning None if credentials are not configured."""
    try:
        return await get_atlassian_service(request)
    except HTTPException as e:
        if e.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR:
            logger.warning(f"Atlassian service not available: {e.detail}")
            return None
        raise
...
async def complete_authorization(
    ...
    This endpoint handles the OAuth callback from Atlassian.
    """
    try:
...
async def get_configuration(
    ...
    try:
        ...
        logger.debug(f"Atlassian config BEFORE sanitize for agent {agent_id}: has_access_token={has_access_token}, connected={config.get('connected')}, access_token present={bool(config.get('access_token'))}")
        ...
        logger.info(f"Atlassian config for agent {agent_id}: connected={sanitized_config.get('connected')}, has_access_token={has_access_token}, config keys={list(sanitized_config.keys())}")
        return success_response(
            data=sanitized_config,
            message="Configuration retrieved successfully"
        )
    except Exception as e:
        logger.error(f"Error getting Atlassian configuration: {e}")
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
async def update_configuration(
    ...
    try:
        ...
        if not stored_config:
            ...
            message="Atlassian integration not found. Please complete OAuth flow first.",
        ...
        logger.info(f"Saved Atlassian configuration for agent {agent_id}: connected={updated_config.get('connected')}")
        ...
        logger.error(f"Error updating Atlassian configuration: {e}", exc_info=True)
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
async def disconnect(
    ...
    try:
        ...
        logger.error(f"Error disconnecting Atlassian: {e}")
        return error_response(
            request=request,
            code=map_status_to_error_code(status.HTTP_500_INTERNAL_SERVER_ERROR),
...
# Fixed OAuth Callback Endpoint (for Atlassian OAuth App redirect)
@callback_router.get(
    "/callback",
    response_model=SuccessResponse[OAuthCallbackResponse],
    responses={
...
async def oauth_callback(
    ...
    try:
        ...
        if not redirect_uri:
            ...
            request=request,
            code=map_status_to_error_code(status.HTTP_400_BAD_REQUEST),
            message="Atlassian OAuth credentials not configured",
            status_code=status.HTTP_400_BAD_REQUEST
        )
