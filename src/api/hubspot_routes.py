...
router = APIRouter(
    prefix="/agents/{agent_id}/integrations/hubspot",
    tags=["hubspot"],
)
...
# Dependency to get HubSpot service
async def get_hubspot_service(
    request: Request
) -> HubSpotService:
    """Get HubSpot service instance with credentials from global config."""
    ...
    credentials = await config_service.get_hubspot_credentials()
    ...
    logger.info(
        ...
        f"client_secret={'***' + client_secret[-4:] if client_secret and len(client_secret) > 4 else 'None'}, "
    ...
    if not client_id or not client_secret or not redirect_uri:
        raise HTTPException(
            ...
            detail="HubSpot OAuth credentials not configured in global_config. "
                   "Please configure: HUBSPOT_OAUTH_CLIENT_ID, HUBSPOT_OAUTH_CLIENT_SECRET, "
                   "HUBSPOT_OAUTH_REDIRECT_URI in the CRM global configuration."
        )
...
async def generate_authorization(
    ...
    """
    try:
        # Ensure OAuth metadata is discovered before generating authorization URL
        if not service._oauth_metadata:
            logger.info("OAuth metadata not cached, discovering OAuth requirements...")
            await service.discover_oauth_requirements()
            logger.info("OAuth requirements discovered successfully")
...
async def complete_authorization(
    ...
    This endpoint handles the OAuth callback from HubSpot.
    """
    try:
        # Load code_verifier from storage before completing authorization
        # HubSpot may use PKCE, so we need to load the code_verifier if it exists
        # Use direct database access instead of HTTP call
        stored_code_verifier = None
        try:
            stored_code_verifier = await service._load_pkce_verifier(
                agent_id=agent_id,
                state=request.state,
                db=db
            )
        ...
        logger.info(
            f"HubSpot callback: agent_id={agent_id}, "
            f"code_length={len(request.code) if request.code else 0}, "
            f"has_code_verifier={bool(stored_code_verifier)}, "
            f"client_id={'***' + service.client_id[-4:] if service.client_id and len(service.client_id) > 4 else 'None'}, "
            f"has_client_secret={bool(service.client_secret)}"
        )
        ...
        logger.info(
            f"HubSpot authorization result: success={result.get('success')}, "
            f"error={result.get('error')}, username={result.get('username')}, "
...
async def get_configuration(
    ...
    try:
        ...
        config = await get_integration_config(db, agent_id, "hubspot")
        
        if not config:
            return success_response(
                data={"provider": "hubspot", "connected": False},
        ...
        logger.debug(f"HubSpot config BEFORE sanitize for agent {agent_id}: has_access_token={has_access_token}, connected={config.get('connected')}, access_token present={bool(config.get('access_token'))}")
        ...
        logger.info(f"HubSpot config for agent {agent_id}: connected={sanitized_config.get('connected')}, has_access_token={has_access_token}, config keys={list(sanitized_config.keys())}")
        return success_response(
...
async def discover_tools(
    ...
    tools = await discover_tools_endpoint(
        agent_id, "hubspot", "https://mcp.hubspot.com/mcp", db, load_from_service=load_from_service
    )
...
async def update_configuration(
    ...
    try:
        ...
        stored_config = await get_integration_config(db, agent_id, "hubspot", load_from_service=service._load_credentials)
        ...
        success = await upsert_agent_integration(
            db, agent_id, "hubspot", updated_config
        )
        ...
        logger.info(f"Saved HubSpot configuration for agent {agent_id}: connected={updated_config.get('connected')}")
...
# Fixed OAuth Callback Endpoint (for HubSpot OAuth App redirect)
@callback_router.get(
    "/callback",
    response_model=SuccessResponse[OAuthCallbackResponse],
    responses={
...
async def oauth_callback(
    ...
    """
    OAuth 2.0 callback endpoint for HubSpot MCP (fixed URL).

    This endpoint handles the OAuth redirect from HubSpot.
    ...
    try:
        ...
        # Get HubSpot service instance (create without user token for callback)
        from src.services.global_config_service import get_global_config_service
